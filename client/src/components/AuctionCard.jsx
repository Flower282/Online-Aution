    ...
    <Link to={`/auction/${auction._id}`} onClick={handleClick}>
      <div className={`bg-white rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer group border flex flex-col h-full ${isEnded
        ...
        <div className="relative w-full h-36 overflow-hidden bg-gray-50 flex-shrink-0">
          ...
          {!isAdmin && (
            <button
              className={`absolute top-2 right-2 p-1.5 rounded-full backdrop-blur-sm transition-all duration-300 ${isLiked
                ? 'bg-cyan-500 hover:bg-cyan-600 text-white scale-110'
          ...
          {isRejected && !isEnded && (
            <div className="absolute bottom-2 left-2 bg-cyan-500 text-white text-xs font-semibold px-2 py-1 rounded-full">
              Rejected
            </div>
          )}
          {isEnded && (
        ...
        {/* Content */}
        <div className="p-3 flex flex-col flex-1 bg-cyan-50/30">
          {/* Title */}
          ...
          <div className="bg-cyan-50 rounded-lg p-2 mb-2">
            ...
            <div className="flex items-center justify-between">
              ...
              {priceIncrease > 0 && (
                <span className="text-sm font-semibold text-cyan-600 flex items-center gap-1">
                  ↗ +{priceIncrease}%
                </span>
              )}
            </div>
          ...
          {!isEnded && !isPending && !isRejected && !isAdmin && (
            <button
              className="w-full bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-2 rounded-lg transition-all duration-300 flex items-center justify-center gap-2 mt-auto text-sm"
              onClick={(e) => {
                if (onClick) {
                  e.preventDefault();
                  handleClick();
